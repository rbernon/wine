...
static struct thread_input *create_thread_input( struct thread *thread )
{
    ...
    if ((input = alloc_object( &thread_input_ops )))
    {
        ...
        if (!(input_shm = get_shared_input( input->session_index )))
        {
            release_object( input );
            return NULL;
        }
...
static struct msg_queue *create_msg_queue( struct thread *thread, struct thread_input *input )
{
    ...
    if ((queue = alloc_object( &msg_queue_ops )))
    {
        ...
        if (!(queue_shm = get_shared_queue( queue->session_index )))
        {
...
static int assign_thread_input( struct thread *thread, struct thread_input *new_input )
{
    ...
    if (queue->input)
    {
        input_shm = get_shared_input( queue->input->session_index );
    ...
    input_shm = get_shared_input( new_input->session_index );
    SHARED_WRITE_BEGIN( input_shm, input_shm_t )
    {
        shared->cursor_count += queue->cursor_count;
    }
...
static int msg_queue_add_queue( struct object *obj, struct wait_queue_entry *entry )
{
    ...
    if (process->idle_event && !(queue_shm->wake_mask & QS_SMRESULT)) set_event( process->idle_event );
...
static void msg_queue_destroy( struct object *obj )
{
    ...
    const input_shm_t *input_shm = get_shared_input( queue->input->session_index );
    int i;
...
int attach_thread_input( struct thread *thread_from, struct thread *thread_to )
{
    ...
    if (thread_from->queue)
    {
        ...
        old_input_shm = get_shared_input( old_input->session_index );
                                   ...
                                   unsigned int origin, struct msg_queue *sender )
{
    ...
    case VK_RMENU:
        if (input->kbd.flags & KEYEVENTF_KEYUP)
        {
            ...
            if (!(desktop_shm->keystate[VK_MENU] & 0x80) || !desktop->last_press_alt) break;
...
DECL_HANDLER(get_message)
{
    ...
    if (!queue) return;
    queue_shm = get_shared_queue( queue->session_index );
...
DECL_HANDLER(get_key_state)
{
    ...
    if (req->async)  /* get global async key state */
    {
        ...
        else set_reply_data( (void *)desktop_shm->keystate, size );
        release_object( desktop );
    }
    else
    {
...
DECL_HANDLER(set_key_state)
{
    ...
    desktop_shm = get_shared_desktop( desktop->session_index );
    memcpy( queue->input->desktop_keystate, (void *)desktop_shm->keystate,
...
DECL_HANDLER(set_foreground_window)
{
    ...
    {
        const input_shm_t *input_shm = get_shared_input( input->session_index );
        reply->previous = input_shm->active;
    }

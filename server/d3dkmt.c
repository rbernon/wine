...
static d3dkmt_handle_t alloc_object_handle( struct d3dkmt_object *object )
{
    ...
    handle = object->global = ((next_index++ << 5) & ~0xc000003f) | 0x40000002;
...
static struct d3dkmt_object *create_d3dkmt_object( enum d3dkmt_type type )
{
...
DECL_HANDLER(create_d3dkmt_object)
{
    ...
    if (!(object = create_d3dkmt_object( req->type ))) return;
...
DECL_HANDLER(share_d3dkmt_objects)
{
    ...
    if (req->resource && !(resource = open_d3dkmt_object( req->resource, D3DKMT_RESOURCE ))) goto done;
    if (req->keyed_mutex && !(keyed_mutex = open_d3dkmt_object( req->keyed_mutex, D3DKMT_KEYED_MUTEX ))) goto done;
    if (req->sync_object && !(sync_object = open_d3dkmt_object( req->sync_object, D3DKMT_KEYED_MUTEX ))) goto done;
...
DECL_HANDLER(get_shared_d3dkmt_object)
{
    ...
    if (req->type == D3DKMT_RESOURCE) object = shared->resource;
    else if (req->type == D3DKMT_KEYED_MUTEX) object = shared->keyed_mutex;
    else if (req->type == D3DKMT_SYNC_OBJECT) object = shared->sync_object;
    else object = NULL;

...
int64_t unix_seek_callback( void *opaque, int64_t offset, int whence )
{
    ...
    for (;;)
    {
        ...
        params.offset = offset % context->capacity;
    ...
    if (offset / context->capacity != context->position / context->capacity) context->size = 0;
...
#endif
{
    ...
    do
    {
        ...
        params.size = min( size, context->size );
...
static void vlog( void *ctx, int level, const char *fmt, va_list va_args )
{
    ...
    if (!NtCurrentTeb()) return; /* ignore log from internal non-Wine threads */

...
trace("SetForegroundWindow desktop\n");
...
trace("IDirect3DDevice9Ex_CheckDeviceState\n");
        hr = IDirect3DDevice9Ex_CheckDeviceState(device, device_window);
        ok(hr == S_PRESENT_OCCLUDED, "Got unexpected hr %#lx, i=%u.\n", hr, i);

trace("EnumDisplaySettingsW\n");
...
trace("ShowWindow SW_MINIMIZE\n");
        ShowWindow(device_window, SW_MINIMIZE);
trace("ShowWindow SW_RESTORE\n");
...
if (devmode.dmPelsHeight != registry_mode.dmPelsHeight) return;
...
trace("SW_SHOWNOACTIVATE\n");
        ShowWindow(focus_window, SW_SHOWNOACTIVATE);
trace("flush_events\n");
        flush_events();
trace("SW_SHOWMINNOACTIVE\n");
        ShowWindow(focus_window, SW_SHOWMINNOACTIVE);
trace("flush_events\n");
        flush_events();

...
gnuc_asinf = np.arcsin(x)
diff = np.float64(gnuc_asinf) - np.arcsin(np.float64(x))
...
utils.plot(diff_df, cmap=list(pal.PuOr9), ax=grid[0], cbax=grid.cbar_axes[0], y_range=(-1,+1))

grid[0].grid(True, alpha=0.1)
grid[0].set_title("GNU libc asinf(x) 1ULP relative error")

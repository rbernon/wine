...
static void tiff_error_handler( const char *module, const char *format, va_list args )
{
    ...
    if (wine_dbg_vlog( __WINE_DBCL_ERR, &__wine_dbch_tiff, __FILE__, __LINE__, module, __builtin_return_address(0), format, args ) != -1)
        __wine_dbg_output( "\n" );
}
...
static void tiff_warning_handler( const char *module, const char *format, va_list args )
{
    ...
    if (wine_dbg_vlog( __WINE_DBCL_WARN, &__wine_dbch_tiff, __FILE__, __LINE__, module, __builtin_return_address(0), format, args ) != -1)
        __wine_dbg_output( "\n" );
}

...
static BOOL X11DRV_ConfigureNotify( HWND hwnd, XEvent *xev )
{
    ...
    UINT flags = 0, old_style, new_style, config_cmd = 0;
    int cx, cy, x, y;
    ...
    if ((old_style & WS_CAPTION) == WS_CAPTION || !data->is_fullscreen)
    {
        ...
        else if ((old_style & WS_MAXIMIZE) && !(new_style & WS_MAXIMIZE))
        {
...
static void handle_wm_state_notify( HWND hwnd, XPropertyEvent *event, BOOL update_window )
{
    ...
    if (!update_window || !data->managed || !(old_style & WS_VISIBLE)) state_cmd = 0;

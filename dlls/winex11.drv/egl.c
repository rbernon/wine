...
static inline GLuint import_texture_resource( HANDLE handle )
{
    ...
    if (desc.type == D3DKMT_UNIX_FD_HANDLE)
    {
        GLint dedicated = 1;
        GLuint memory;
    ...
    else if (desc.type == D3DKMT_DMA_BUF_HANDLE)
    {
...
{
    d3dkmt_desc_t desc = {.type = D3DKMT_DMA_BUF_HANDLE};
...
.initialLayout = 0,
    ...
    d3dkmt_desc_t desc = {.type = D3DKMT_UNIX_FD_HANDLE};
    VkMemoryRequirements requirements = {0};
    VkDeviceMemory memory;
    VkImage image;
    UINT status;

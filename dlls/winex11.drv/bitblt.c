...
ERR("window_surface %p hwnd %p/%lx rect %s dirty %s shape %u\n", window_surface, window_surface->hwnd, surface->window, wine_dbgstr_rect(rect), wine_dbgstr_rect(dirty), shape_changed);

    sprintf(path, "/tmp/surface-x11drv-%p-%u.bmp", window_surface, count++);
    file = fopen(path, "w");

...
void wineXmlCallbackLog(char const* caller, xmlErrorLevel lvl, char const* msg, va_list ap)
{
    ...
    wine_dbg_log(dbcl, &__wine_dbch_msxml, __FILE__, __LINE__, caller,
                 __builtin_extract_return_addr(__builtin_return_address(0)), "%s", buff);
}
...
void wineXmlCallbackError(char const* caller, xmlErrorPtr err)
{
    ...
    wine_dbg_log(dbcl, &__wine_dbch_msxml, __FILE__, __LINE__, caller,
                 __builtin_extract_return_addr(__builtin_return_address(0)), "error code %d", err->code);
    if (err->message)
        wine_dbg_log(dbcl, &__wine_dbch_msxml, __FILE__, __LINE__, caller,
                     __builtin_extract_return_addr(__builtin_return_address(0)), ": %s", err->message);
    else
        wine_dbg_log(dbcl, &__wine_dbch_msxml, __FILE__, __LINE__, caller,

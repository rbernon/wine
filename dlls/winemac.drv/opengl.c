...
static GLuint import_texture_io_surface_pixb( CGLContextObj context, const struct io_surface_desc *desc )
{
    CVOpenGLTextureCacheRef cache;
    CVOpenGLTextureRef texture;
    CVPixelBufferRef buffer;

    CVPixelBufferCreateWithIOSurface(kCFAllocatorDefault, desc->surface, NULL, &buffer);
    CVOpenGLTextureCacheCreate(kCFAllocatorDefault, NULL, context, desc->format, NULL, &cache);
    CVOpenGLTextureCacheCreateTextureFromImage(kCFAllocatorDefault, cache, buffer, NULL, &texture);
    /* FIXME: CFRelease */

    return CVOpenGLTextureGetName(texture);
}

static GLuint import_texture_io_surface_bind( CGLContextObj context, const struct io_surface_desc *desc )
{
    GLuint texture;

    glGenTextures( 1, &texture );
    glBindTexture( GL_TEXTURE_2D, texture );
    CGLTexImageIOSurface2D( context, GL_TEXTURE_2D, GL_RGBA8, desc->width, desc->height, GL_RGBA8, GL_RGBA, desc->surface, 0 );

    return texture;
}

static VkImage import_image_io_surface( VkDevice device, const struct io_surface_desc *desc )
{
    ...
    VkImportMetalIOSurfaceInfoEXT external =
    {
        .sType = VK_STRUCTURE_TYPE_IMPORT_METAL_IO_SURFACE_INFO_EXT,
        .ioSurface = desc->surface,
    };
...
static GLuint create_texture_io_surface( CGLContextObj context, int width, int height, struct io_surface_desc *desc )
{
    CVOpenGLTextureCacheRef cache;
    ...
    CFDictionarySetValue(attrs, kCVPixelBufferIOSurfacePropertiesKey, kCFBooleanTrue);
    ...
    CVPixelBufferCreate(kCFAllocatorDefault, width, height, GL_RGBA8 /*FIXME*/, attrs, &buffer);
    CFRelease(attrs);
...
static VkImage create_image_io_surface( VkDevice device, int width, int height, struct io_surface_desc *desc )
{
    CFMutableDictionaryRef attrs;
    CVPixelBufferRef buffer;
    ...
    CFDictionarySetValue(attrs, kCVPixelBufferMetalCompatibilityKey, kCFBooleanTrue);
    CFDictionarySetValue(attrs, kCVPixelBufferIOSurfacePropertiesKey, kCFBooleanTrue);
    CFDictionarySetValue(attrs, kCVPixelBufferIOSurfaceOpenGLTextureCompatibilityKey, kCFBooleanTrue);
    CFDictionarySetValue(attrs, kCVPixelBufferIOSurfaceOpenGLFBOCompatibilityKey, kCFBooleanTrue);
    CFDictionarySetValue(attrs, kCVPixelBufferIOSurfaceCoreAnimationCompatibilityKey, kCFBooleanTrue);
    CVPixelBufferCreate(kCFAllocatorDefault, width, height, GL_RGBA8 /*FIXME*/, attrs, &buffer);
    CFRelease(attrs);

    desc->surface = CVPixelBufferGetIOSurface(buffer);
    desc->width = width;
    desc->height = height;

    return import_image_io_surface(device, desc);
}

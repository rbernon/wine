        ...
        MediaEventType expect_type, DWORD timeout, PROPVARIANT *value)
{
    ...
    ok_(file, line)(hr == S_OK, "Unexpected hr %#lx.\n", hr);
    ok_(file, line)(IsEqualGUID(&guid, &GUID_NULL), "got extended type %s\n", debugstr_guid(&guid));
...
IMFMediaSource *create_media_source(const WCHAR *name, const WCHAR *mime)
{
...
static void test_media_session_transform_multiple_streams(void)
{
    IMFAsyncCallback *media_event_callback, *async_callbacks[2];
    IMFSampleGrabberSinkCallback *grabber_callback;
    IMFTopologyNode *up_nodes[2], *node;
    IMFMediaType *media_types[2];
    UINT32 i, count, queues[2];
    ...
    grabber_callback = create_test_grabber_callback();
    media_event_callback = create_test_callback(TRUE);
    async_callbacks[0] = create_test_callback(FALSE);
    async_callbacks[1] = create_test_callback(FALSE);

    hr = MFCreateMediaSession(NULL, &session);
    ok(hr == S_OK, "got hr %#lx\n", hr);


    /* create the topology with 2x 1 source -> MFT -> 1 output */
    hr = MFCreateTopology(&topology);
    ok(hr == S_OK, "got hr %#lx\n", hr);
    ...
    node = test_transform_create_transform_node(topology, 1, media_types, 1, media_types, &transforms[0]);
    hr = IMFTopologyNode_ConnectOutput(up_nodes[0], 0, node, 0);
    ok(hr == S_OK, "got hr %#lx\n", hr);
    IMFTopologyNode_Release(up_nodes[0]);
    up_nodes[0] = node;
    ...
    node = test_transform_create_transform_node(topology, 1, media_types + 1, 1, media_types + 1, &transforms[1]);
    hr = IMFTopologyNode_ConnectOutput(up_nodes[1], 0, node, 0);
    ok(hr == S_OK, "got hr %#lx\n", hr);
    IMFTopologyNode_Release(up_nodes[1]);
    up_nodes[1] = node;
    ...
    res = test_transform_wait_process_input(transforms[0], 0);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    res = test_transform_wait_process_input(transforms[1], 0);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    res = test_transform_wait_process_output(transforms[0], 1000);
    ok(res == 0, "got res %#lx\n", res);
    res = test_transform_wait_process_output(transforms[1], 1000);
    ok(res == 0, "got res %#lx\n", res);

    /* work queues are now blocked */
    hr = MFPutWorkItem(queues[0], async_callbacks[0], NULL);
    ok(hr == S_OK, "got hr %#lx\n", hr);
    res = wait_async_callback(async_callbacks[0], 100);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    hr = MFPutWorkItem(queues[1], async_callbacks[1], NULL);
    ok(hr == S_OK, "got hr %#lx\n", hr);
    res = wait_async_callback(async_callbacks[1], 100);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    ...
    for (i = 0, time = 0, duration = 1000; i < 20; i++)
    {
        ...
        test_transform_set_output(transforms[0], 0, sample);
        test_transform_set_output(transforms[1], 0, sample);

        /* work queues are now unblocked */
        res = wait_async_callback(async_callbacks[0], 1000);
        ok(res == 0, "got res %#lx\n", res);
        res = wait_async_callback(async_callbacks[1], 1000);
        ok(res == 0, "got res %#lx\n", res);
        ...
        hr = MFPutWorkItem(queues[0], async_callbacks[0], NULL);
        ok(hr == S_OK, "got hr %#lx\n", hr);
        res = wait_async_callback(async_callbacks[0], 0);
        ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
        hr = MFPutWorkItem(queues[1], async_callbacks[1], NULL);
        ok(hr == S_OK, "got hr %#lx\n", hr);
        res = wait_async_callback(async_callbacks[1], 0);
        ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    ...
    res = test_transform_wait_process_input(transforms[1], 0);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    res = test_transform_wait_process_output(transforms[0], 0);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    res = test_transform_wait_process_output(transforms[1], 0);
    ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
    test_transform_set_output(transforms[0], 0, NULL);
    test_transform_set_output(transforms[1], 0, NULL);
    ...
    for (i = 0; i < 10; i++)
    {
        ...
        /* a single ProcessInput call for stream 0 or 1, depending on the media source */
        res = test_transform_wait_process_input(transforms[0], 1000);
        ok(res == 0, "got res %#lx\n", res);
        res = test_transform_wait_process_input(transforms[1], 1000);
        ok(res == 0, "got res %#lx\n", res);
        res = test_transform_wait_process_input(transforms[0], 0);
        ok(res == WAIT_TIMEOUT, "got res %#lx\n", res);
        res = test_transform_wait_process_input(transforms[1], 0);
        ...
        /* take the sample and return S_OK */
        test_transform_get_input(transforms[0], 0, &sample);
        test_transform_get_input(transforms[1], 0, &sample);
        ...
        /* release the sample and return MF_E_NOTACCEPTING */
        test_transform_get_input(transforms[0], 0, NULL);
        test_transform_get_input(transforms[1], 0, NULL);
    ...
    test_transform_shutdown(transforms[0]);
    test_transform_shutdown(transforms[1]);
    ...
    IMFAsyncCallback_Release(media_event_callback);
    IMFAsyncCallback_Release(async_callbacks[0]);
    IMFAsyncCallback_Release(async_callbacks[1]);
...
START_TEST(mf)
{
    ...
    test_media_session_transform_multiple_streams();

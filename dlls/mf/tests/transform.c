...
void dump_attributes_(int line, IMFAttributes *attributes)
{
    ...
    for (i = 0; i < count; ++i)
    {
        ...
        switch (value.vt)
        {
        default: sprintf(buffer, "{%s, .vt = %u},", debugstr_mf_guid(&guid), value.vt); break;
        ...
        case VT_UNKNOWN: sprintf(buffer, "ATTR_IFACE(%s, %p),", debugstr_mf_guid(&guid), value.punkVal); break;

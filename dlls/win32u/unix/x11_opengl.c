...
static RTL_CRITICAL_SECTION context_section;
...
static void update_hwnd_gl_drawable(HWND hwnd, struct gl_drawable *gl)
{
    ...
    RtlEnterCriticalSection( &context_section );
    if (gl) new_entry = &grab_gl_drawable(gl)->hwnd_entry;
    if ((entry = wine_rb_get( &hwnd_drawables, hwnd )))
    {
...
static BOOL X11DRV_WineGL_InitOpenglInfo(void)
{
    ...
    if (vis->visual != DefaultVisual( gdi_display, vis->screen ))
        attr.colormap = pXCreateColormap( gdi_display, root, vis->visual, AllocNone );
    if ((win = pXCreateWindow( gdi_display, root, -1, -1, 1, 1, 0, vis->depth, InputOutput,
                              vis->visual, CWBorderPixel | CWOverrideRedirect | CWColormap, &attr )))
        pXMapWindow( gdi_display, win );
    else
        win = root;
    ...
    glExtensions = RtlAllocateHeap(GetProcessHeap(), 0, strlen(str)+sizeof(legacy_extensions));
    ...
    if (win != root) pXDestroyWindow( gdi_display, win );
    if (attr.colormap) pXFreeColormap( gdi_display, attr.colormap );
...
#undef LOAD_FUNCPTR
    ...
    if (pXQueryExtension( gdi_display, "GLX", &glx_opcode, &event_base, &error_base ))
    {
...
static void init_pixel_formats( Display *display )
{
    ...
    list = RtlAllocateHeap(GetProcessHeap(), HEAP_ZERO_MEMORY, (nCfgs + bmp_formats) * sizeof(*list));
    ...
    for(run=0; run < 2; run++)
    {
        for(i=0; i<nCfgs; i++) {
            ...
            else if (visinfo) pXFree(visinfo);
        }
    }
...
static struct gl_drawable *create_gl_drawable( HWND hwnd, const struct wgl_pixel_format *format, BOOL known_child )
{
    ...
    if (!(gl = RtlAllocateHeap( GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*gl) ))) return NULL;
    ...
    else
    {
        ...
        gl->pixmap = pXCreatePixmap( gdi_display, root_window, width, height, visual->depth );
        if (gl->pixmap)
        {
            gl->drawable = pglXCreatePixmap( gdi_display, gl->format->fbconfig, gl->pixmap, NULL );
            if (!gl->drawable) pXFreePixmap( gdi_display, gl->pixmap );
...
static struct wgl_context * WINAPI glxdrv_wglCreateContext( HDC hdc )
{
    ...
    if ((ret = RtlAllocateHeap(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*ret))))
    {
        ret->hdc = hdc;
        ret->fmt = gl->format;
...
static BOOL WINAPI glxdrv_wglDeleteContext(struct wgl_context *ctx)
{
    ...
    release_gl_drawable( ctx->new_drawables[0] );
    release_gl_drawable( ctx->new_drawables[1] );
    return RtlFreeHeap( GetProcessHeap(), 0, ctx );
}
                                                       ...
                                                       const int *piAttribList )
{
    ...
    object = RtlAllocateHeap(GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*object));
    if (NULL == object) {
...
static HDC X11DRV_wglGetPbufferDCARB( struct wgl_pbuffer *object )
{
    ...
    if (!(gl = RtlAllocateHeap( GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*gl) )))
    {
        DeleteDC( hdc );
        return 0;
    }
                                                 ...
                                                 UINT nAttributes, const int *piAttributes, FLOAT *pfValues )
{
    ...
    attr = RtlAllocateHeap(GetProcessHeap(), 0, nAttributes * sizeof(int));
    if (!attr) {
    ...
    RtlFreeHeap(GetProcessHeap(), 0, attr);
    return ret;
}

...
{
    return wine_dbg_sprintf( "base %p, rect %s, bounds %s, hwnd %p, window %lx, window_surface %p, image_surface %p",
                             &impl->base, wine_dbgstr_rect(&impl->base.rect), wine_dbgstr_rect(&impl->bounds), impl->hwnd,
                             impl->window, impl->window_surface, impl->image_surface );
}
...
static cairo_status_t dump_surface_callback(void *closure, const unsigned char *data, unsigned int length)
{
    fwrite(data, 1, length, closure);
    return CAIRO_STATUS_SUCCESS;
}

static inline void dump_surface( struct x11_window_surface *impl )
{
    char buffer[1024];
    sprintf(buffer, "/tmp/win32u/win32u-surface-%p-%d.png", impl, impl->flushes);
    FILE *f = fopen(buffer, "w+");
    p_cairo_surface_write_to_png_stream( impl->image_surface, dump_surface_callback, f );
    fclose(f);
}
...
static void CDECL x11_window_surface_lock( struct window_surface *base )
{
    ...
    if (!impl->window_surface || impl->image_surface) return;
...
static void *CDECL x11_window_surface_get_info( struct window_surface *base, BITMAPINFO *info )
{
    ...
    if (impl->window_surface && !impl->info.biSizeImage)
    {
        ...
        impl->info.biSizeImage = p_cairo_image_surface_get_height( surface ) * p_cairo_image_surface_get_stride( surface );
...
static void CDECL x11_window_surface_flush( struct window_surface *base )
{
    ...
    if (impl->image_surface)
    {
        ...
        p_cairo_surface_mark_dirty_rectangle( impl->image_surface, cairo_rect.x, cairo_rect.y, cairo_rect.width, cairo_rect.height );
...
static void CDECL x11_window_surface_destroy( struct window_surface *base )
{
    ...
    if (impl->image_surface) p_cairo_surface_unmap_image( impl->window_surface, impl->image_surface );
...
struct window_surface* CDECL x11_create_window_surface(const RECT *screen_rect, const RECT *visible_rect, HWND hwnd, UINT64 unix_handle)
{
    ...
    if ((impl = RtlAllocateHeap( GetProcessHeap(), HEAP_ZERO_MEMORY, sizeof(*impl) )))
    {
        ...
        if (impl->window && !IsRectEmpty(&impl->base.rect) && pXGetWindowAttributes(gdi_display, impl->window, &attr))
{
ERR("%s:%d\n", __FILE__, __LINE__);
...
struct window_surface *CDECL x11_resize_window_surface(struct window_surface *base, const RECT *screen_rect, const RECT *visible_rect, HWND hwnd, UINT64 unix_handle)
{
    ...
    {
        RtlEnterCriticalSection(&impl->cs);
        if (!impl->window_surface && impl->window && !IsRectEmpty(&impl->base.rect) && pXGetWindowAttributes(gdi_display, impl->window, &attr))
{
ERR("%s:%d\n", __FILE__, __LINE__);
            impl->window_surface = p_cairo_xlib_surface_create( gdi_display, impl->window, attr.visual, attr.width, attr.height );
            /*p_cairo_xlib_surface_set_size(impl->window_surface, attr.width, attr.height);*/

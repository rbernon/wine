...
BOOL CDECL WIN32U_CreateWindow( HWND hwnd )
{
    ...
    if (!desktop_ioctl( IOCTL_WIN32U_CREATE_WINDOW, &in, sizeof(in), NULL, 0, NULL )) return FALSE;
                                     ...
                                     struct window_surface **surface, RECT *screen_rect, void **driver_handle )
{
    ...
    TRACE( "hwnd %p, insert_after %p, swp_flags %x, window_rect %s, client_rect %s, visible_rect "
           ...
           hwnd, insert_after, swp_flags, wine_dbgstr_rect( window_rect ),
           wine_dbgstr_rect( client_rect ), visible_rect, surface, screen_rect, driver_handle );
    ...
    TRACE( "screen_rect %s, visible_rect %s, unix_handle %lx.\n", wine_dbgstr_rect( &out.screen_rect ),
           wine_dbgstr_rect( &out.visible_rect ), (long int)out.unix_handle );

    if (out.unix_handle)
    {
        if (!*surface)
            *surface = unix_funcs->create_window_surface(
                &out.screen_rect, &out.visible_rect, hwnd, out.unix_handle );
        else
            *surface = unix_funcs->resize_window_surface(
                *surface, &out.screen_rect, &out.visible_rect, hwnd, out.unix_handle );
    }
...
void CDECL WIN32U_WindowPosChanged( HWND hwnd, HWND insert_after, UINT swp_flags, const RECT *window_rect,
                                    const RECT *client_rect, const RECT *visible_rect,
...
void CDECL WIN32U_SetWindowStyle( HWND hwnd, INT offset, STYLESTRUCT *style )
{
    ...
    if (!desktop_ioctl( IOCTL_WIN32U_SET_WINDOW_STYLE, &in, sizeof(in), NULL, 0, NULL )) return;
}
...
void CDECL WIN32U_SetFocus( HWND hwnd )
{
    ...
    if (!desktop_ioctl( IOCTL_WIN32U_SET_FOCUS, &in, sizeof(in), NULL, 0, NULL )) return;
}
                              ...
                              int buf_size, UINT flags, HKL hkl )
{
    ...
    if (!desktop_ioctl( IOCTL_WIN32U_TO_UNICODE, &in, sizeof(in), buf, buf_size, &ret )) return 0;

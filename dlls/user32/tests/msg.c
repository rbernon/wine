...
static void test_interthread_set_foreground_window(struct test_set_foreground_window_args *args, HWND hwnd0, HWND hwnd1, HWND hwnd2)
{
    ...
    ok( res == WAIT_OBJECT_0, "WaitForSingleObject returned %#lx, last error %#lx.\n", res, GetLastError() );
    ...
    for (i = 0; i < args->tests_count; ++i)
    {
        ...
        winetest_push_context("test %ld", i);
...
static void test_SetForegroundWindow(void)
{
    ...
    ok( !!args.ready, "CreateEvent failed, last error %#lx.\n", GetLastError() );
    args.start = CreateEventA( NULL, FALSE, FALSE, NULL );
    ok( !!args.start, "CreateEvent failed, last error %#lx.\n", GetLastError() );
    args.done = CreateEventA( NULL, FALSE, FALSE, NULL );
    ok( !!args.done, "CreateEvent failed, last error %#lx.\n", GetLastError() );
    ...
    ok( res, "SetEvent failed, last error %#lx.\n", GetLastError() );

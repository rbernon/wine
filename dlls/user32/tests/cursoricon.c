...
static LRESULT CALLBACK callback_child(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg)
    {
        case WM_USER+2:
        {
            ...
            ok(GetLastError() == 0xdeadbeef, "unexpected error %lu\n", GetLastError());
        ...
        case WM_USER+1:
        {
            ...
            todo_wine ok(pixels[0] == 0x03020100, "pixels[0] is 0x%08lx\n", pixels[0]);
            todo_wine ok(pixels[16] == 0xcdcdcdcd, "pixels[16] is 0x%08lx\n", pixels[16]);
            ReleaseDC(NULL, hdc);

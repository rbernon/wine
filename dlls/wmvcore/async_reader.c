...
static HRESULT async_reader_get_next_sample(struct async_reader *reader, DWORD stream_count,
...
static void async_reader_deliver_samples(struct async_reader *reader, DWORD stream_count)
{
    ...
    while (reader->running && list_empty(&reader->async_ops))
    {
        ...
        if (FAILED(hr = async_reader_get_next_sample(reader, stream_count,
                &stream, &sample)))
            break;

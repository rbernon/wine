...
VkResult (WINAPI *p_vkCreateInstance)(const VkInstanceCreateInfo *create_info,
        const VkAllocationCallbacks *allocator, VkInstance *ret);
VkResult (WINAPI *p_vkCreateDevice)(VkPhysicalDevice phys_dev, const VkDeviceCreateInfo *create_info,
...
static void load_vulkan(void)
{
    ...
    h = LoadLibraryA("vulkan-1");
    if(!h){
        WINE_ERR("unable to load winevulkan\n");
        return;
    }

    p_vkCreateInstance = (void *)GetProcAddress(h, "vkCreateInstance");
    p_vkCreateDevice = (void *)GetProcAddress(h, "vkCreateDevice");
}

static uint64_t get_client_physical_device( VkInstance client_instance, VkPhysicalDevice host_physical_device )
{
    ...
    for (i = 0; i < instance->physical_device_count; i++)
    {
        ...
        if (physical_device->host.physical_device == host_physical_device) return physical_device->client.physical_device;
    }
    ...
    if(createInfo->next){
        switch(((XrBaseInStructure *)createInfo->next)->type){
            ...
            case XR_TYPE_GRAPHICS_BINDING_D3D11_KHR:
            {
                ...
                our_vk_binding.instance = vulkan_instance_from_handle(our_vk_binding.instance)->host.instance;

                if ((res = do_vulkan_init(wine_instance, our_vk_binding.instance)) != XR_SUCCESS)
                    return res;

                if (wine_instance->vk_phys_dev != vulkan_physical_device_from_handle(our_vk_binding.physicalDevice)->host.physical_device)
                ...
                our_vk_binding.device = vulkan_device_from_handle(our_vk_binding.device)->host.device;
            ...
            case XR_TYPE_GRAPHICS_BINDING_D3D12_KHR:
            {
                ...
                wine_instance->vk_device = our_vk_binding.device = vulkan_device_from_handle(our_vk_binding.device)->host.device;
                wine_instance->vk_queue = vulkan_queue_from_handle(wine_instance->vk_queue)->host.queue;
        ...
        XrSystemId systemId, VkInstance vkInstance, VkPhysicalDevice *vkPhysicalDevice)
{
    ...
    res = ((wine_XrInstance *)instance)->funcs.p_xrGetVulkanGraphicsDeviceKHR(((wine_XrInstance *)instance)->instance, systemId, vulkan_instance_from_handle(vkInstance)->host.instance, vkPhysicalDevice);
        ...
        VkInstance *vulkanInstance, VkResult *vulkanResult)
{
    ...
    ret = *vulkanResult = p_vkCreateInstance(&createInfo->vulkanCreateInfo, createInfo->vulkanAllocator, vulkanInstance);
    if (!*vulkanResult)
    {
        struct vulkan_instance *instance = vulkan_instance_from_handle(*vulkanInstance);
        XrVulkanInstanceCreateInfoKHR our_create_info;
        VkInstanceCreateInfo our_vulkan_create_info;
        const char **enabled_extensions = NULL;
        unsigned int i;

        enabled_extensions = malloc(instance->extension_count * sizeof(*enabled_extensions));
        for (i = 0; i < instance->extension_count; ++i) enabled_extensions[i] = instance->extensions[i].extensionName;

        our_vulkan_create_info = *createInfo->vulkanCreateInfo;
        our_vulkan_create_info.enabledExtensionCount = instance->extension_count;
        our_vulkan_create_info.ppEnabledExtensionNames = enabled_extensions;

        our_create_info = *createInfo;
        our_create_info.pfnGetInstanceProcAddr = instance->host_vkGetInstanceProcAddr;
        our_create_info.vulkanCreateInfo = &our_vulkan_create_info;

        ret = ((wine_XrInstance *)instance)->funcs.p_xrCreateVulkanInstanceKHR(((wine_XrInstance *)instance)->instance, &our_create_info, vk_instance, &ret);
        free(enabled_extensions);
    }

    if (ret == XR_SUCCESS && *vulkanResult != VK_SUCCESS)
...
XrResult WINAPI wine_xrCreateVulkanDeviceKHR(XrInstance instance, const XrVulkanDeviceCreateInfoKHR *createInfo, VkDevice *vulkanDevice, VkResult *vulkanResult)
{
    ...
    *vulkanResult = p_vkCreateDevice(createInfo->vulkanPhysicalDevice, &createInfo->vulkanCreateInfo,
            createInfo->vulkanAllocator, vulkanDevice);
    if (!*vulkanResult)
    {
        struct vulkan_device *device = vulkan_device_from_handle(*vulkanDevice);
        ...
        vulkan_create_info.enabledExtensionCount = instance->extension_count;
        vulkan_create_info.ppEnabledExtensionNames = enabled_extensions;
        ...
        ret = ((wine_XrInstance *)instance)->funcs.p_xrCreateVulkanDeviceKHR(((wine_XrInstance *)instance)->instance, &our_create_info, vk_device, &ret);
        free(enabled_extensions);
    }

...
#define USE_XR_FUNC(name) \
    if ((res = p_xrGetInstanceProcAddr(instance->host.instance, #name, (PFN_xrVoidFunction *)&instance->p_##name))) goto failed;
...
XrResult wine_xrCreateVulkanDeviceKHR(XrInstance client_instance, const XrVulkanDeviceCreateInfoKHR *create_info, VkDevice *vulkan_device, VkResult *vulkan_result)
{
    return XR_ERROR_FUNCTION_UNSUPPORTED;
}

XrResult wine_xrCreateVulkanInstanceKHR(XrInstance client_instance, const XrVulkanInstanceCreateInfoKHR *create_info, VkInstance *vulkan_instance, VkResult *vulkan_result)
{
    return XR_ERROR_FUNCTION_UNSUPPORTED;
}

XrResult wine_xrGetVulkanGraphicsDeviceKHR(XrInstance client_instance, XrSystemId system_id, VkInstance vulkan_instance, VkPhysicalDevice *vulkan_physical_device)
{
    return XR_ERROR_FUNCTION_UNSUPPORTED;
}

XrResult wine_xrGetVulkanGraphicsDevice2KHR(XrInstance client_instance, const XrVulkanGraphicsDeviceGetInfoKHR *get_info, VkPhysicalDevice *vulkan_physical_device)
{
    return XR_ERROR_FUNCTION_UNSUPPORTED;
}
...
XrResult wine_xrCreateReferenceSpace(XrSession client_session, const XrReferenceSpaceCreateInfo *create_info, XrSpace *ret)
{
    struct openxr_session *session = openxr_session_from_handle(client_session);
    struct openxr_instance *instance = session->instance;
    XrSpace host_space, client_space = *ret;

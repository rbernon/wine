...
static char prof_data_pool[256];
static size_t prof_data_pool_size;

struct __wine_prof_data *__cdecl __wine_prof_data_alloc( size_t size )
{
    size_t alloc_size = sizeof(struct __wine_prof_data) + size;
    alloc_size = __atomic_fetch_add( &prof_data_pool_size, alloc_size, __ATOMIC_SEQ_CST );
    if (alloc_size < ARRAY_SIZE(prof_data_pool)) return (struct __wine_prof_data *)(prof_data_pool + alloc_size - size) - 1;

                       ...
                       const char *file, int line, const char *function, void *retaddr )
{
    ...
    if (TRACE_ON(pid)) pos += sprintf( pos, "%04x:%9u:", GetCurrentProcessId(), (UINT)(ULONG_PTR)NtCurrentTeb()->SystemReserved1[0] );
    pos += sprintf( pos, "%04x:%9u:", GetCurrentThreadId(), (UINT)(ULONG_PTR)NtCurrentTeb()->SystemReserved1[1] );

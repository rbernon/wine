...
static void read_file( const WCHAR *filename, unsigned char **data )
{
    ...
    file = CreateFileW( filename, GENERIC_READ, FILE_SHARE_READ, NULL, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, NULL );
...
static void test_idl_parsing(void)
{
    ...
    struct output header = {.name = L"main.h", .flag = strcmp( winetest_platform, "wine" ) ? L"-h" : L"-H"};

...
{
    HHOOK cbt_hook, mouse_hook = NULL, keyboard_hook = NULL;
    typeof(hcbt_hook) *cbt_hook_proc;
    WNDCLASSEXW class = {
    ...
    if (!wcsstr( cmdline, L"hcbt" )) cbt_hook = NULL;
    else
    {
        wintrace_hooks = LoadLibraryW( L"wintrace_hooks" );
        cbt_hook_proc = (void *)GetProcAddress( wintrace_hooks, "hcbt_hook" );
        cbt_hook = SetWindowsHookExW( WH_CBT, cbt_hook_proc, wintrace_hooks, 0 );
        assert( cbt_hook );
    }

    if (wcsstr( cmdline, L"ll" ))
    {

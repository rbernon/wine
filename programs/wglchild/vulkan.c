...
#define ERR_EXIT( err_msg, err_class )                                                             \
    do                                                                                             \
    {                                                                                              \
        if (!demo->suppress_popups) MessageBoxW( NULL, L ## err_msg, L ## err_class, MB_OK );      \
        exit( 1 );                                                                                 \
    } while (0)
...
#define GET_INSTANCE_PROC_ADDR( inst, entrypoint )                                                 \
    {                                                                                              \
        demo->fp##entrypoint = (PFN_vk##entrypoint)vkGetInstanceProcAddr( inst,                    \
                                                                          "vk" #entrypoint );      \
...
#define GET_DEVICE_PROC_ADDR( dev, entrypoint )                                                    \
    {                                                                                              \
        if (!g_gdpa)                                                                               \
            g_gdpa = (PFN_vkGetDeviceProcAddr)vkGetInstanceProcAddr( demo->inst,                   \
                                                                     "vkGetDeviceProcAddr" );      \
        ...
        for (uint32_t object = 0; object < pCallbackData->objectCount; ++object)
        {
                ...
                strlen( pCallbackData->pObjects[object].pObjectName ) > 0)
            {
                sprintf( tmp_message, "\t\tObject[%d] - %s, Handle %p, Name \"%s\"\n", object,
                         ...
                         (void *)(UINT_PTR)( pCallbackData->pObjects[object].objectHandle ),
...
static void demo_set_image_layout( struct demo *demo, VkImage image, VkImageAspectFlags aspectMask,
                                   VkImageLayout old_image_layout, VkImageLayout new_image_layout, VkAccessFlagBits srcAccessMask,
                                   VkPipelineStageFlags src_stages, VkPipelineStageFlags dest_stages )
{
    ...
    default: image_memory_barrier.dstAccessMask = 0; break;
    }
...
static void demo_draw_build_cmd( struct demo *demo, VkCommandBuffer cmd_buf )
{
    ...
    if (demo->validate)
    {
        ...
        VkDebugUtilsObjectNameInfoEXT cmd_buf_name =
        {
            ...
            .objectHandle = (uint64_t)( UINT_PTR)cmd_buf,
    ...
    if (demo->separate_present_queue)
    {
        ...
        vkCmdPipelineBarrier( cmd_buf, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT, VK_PIPELINE_STAGE_BOTTOM_OF_PIPE_BIT,
...
void demo_update_data_buffer( struct demo *demo )
{
    ...
    memcpy( demo->swapchain_image_resources[demo->current_buffer].uniform_memory_ptr,
            (const void *)&MVP[0][0], matrixSize );
}
...
void demo_prepare_cube_data_buffers( struct demo *demo )
{
    ...
    for (unsigned int i = 0; i < demo->swapchainImageCount; i++)
    {
        ...
        err = vkMapMemory( demo->device, demo->swapchain_image_resources[i].uniform_memory, 0,
                           VK_WHOLE_SIZE, 0, &demo->swapchain_image_resources[i].uniform_memory_ptr );
        assert( !err );
...
void vulkan_draw_frame( HWND hwnd )
{
    struct demo *data = (void *)GetWindowLongPtrW( hwnd, GWLP_USERDATA );
    if (data) demo_draw( data );
}

// MS-Windows event handling function:
LRESULT CALLBACK vulkan_window_proc( HWND hwnd, UINT msg, WPARAM wparam, LPARAM lparam )
{
    struct demo *demo = (struct demo *)GetWindowLongPtrW( hwnd, GWLP_USERDATA );
    ...
    case WM_KEYDOWN:
        ...
        switch (wparam)
        {
        case VK_ESCAPE: PostQuitMessage( validation_error ); break;
...
static void demo_create_xlib_window( struct demo *demo )
{
    ...
    Colormap colormap = XCreateColormap( demo->display, RootWindow( demo->display, vInfoTemplate.screen ),
    ...
    demo->xlib_window = XCreateWindow( demo->display, RootWindow( demo->display, vInfoTemplate.screen ),
                                       0, 0, demo->width, demo->height, 0, visualInfo->depth, InputOutput,
                                       visualInfo->visual, CWBackPixel | CWBorderPixel | CWEventMask | CWColormap,
                                       &windowAttributes );
...
static void demo_create_xcb_window( struct demo *demo )
{
    ...
    xcb_create_window( demo->connection, XCB_COPY_FROM_PARENT, demo->xcb_window, demo->screen->root,
                       0, 0, demo->width, demo->height, 0, XCB_WINDOW_CLASS_INPUT_OUTPUT,
    ...
    xcb_change_property( demo->connection, XCB_PROP_MODE_REPLACE, demo->xcb_window, (*reply).atom,
                         4, 32, 1, &(*demo->atom_wm_delete_window).atom );
    free( reply );
...
static void handle_toplevel_configure( void *data, struct xdg_toplevel *xdg_toplevel UNUSED,
                                       int32_t width, int32_t height, struct wl_array *states UNUSED )
{
    struct demo *demo = (struct demo *)data;
...
static void demo_create_window( struct demo *demo )
{
    ...
    if (demo->xdg_decoration_mgr)
    {
        ...
        demo->toplevel_decoration = zxdg_decoration_manager_v1_get_toplevel_decoration( demo->xdg_decoration_mgr, demo->xdg_toplevel );
...
int find_display_gpu( int gpu_number, uint32_t gpu_count, VkPhysicalDevice *physical_devices )
{
    ...
    if (display_count > 0) return gpu_return;
    else return -1;
}
#endif
static void demo_init_vk( struct demo *demo )
{
    ...
    if (demo->validate)
    {
        ...
        if (!validation_found)
        {
            ERR_EXIT( "vkEnumerateInstanceLayerProperties failed to find required validation "
                      "layer.\n\n"
    ...
    if (instance_extension_count > 0)
    {
        ...
        for (uint32_t i = 0; i < instance_extension_count; i++)
        {
            ...
            // We want cube to be able to enumerate drivers that support the portability_subset
            // extension, so we have to enable the portability enumeration extension.
...
#elif defined( VK_USE_PLATFORM_METAL_EXT )
        ERR_EXIT( "vkEnumerateInstanceExtensionProperties failed to find "
                  "the " VK_EXT_METAL_SURFACE_EXTENSION_NAME " extension.\n\n"
                  "Do you have a compatible Vulkan installable client driver (ICD) installed?\n"
                  "Please look at the Getting Started guide for additional information.\n",
                  "vkCreateInstance Failure" );
#elif defined( VK_USE_PLATFORM_XCB_KHR )
...
#elif defined( VK_USE_PLATFORM_ANDROID_KHR )
        ERR_EXIT( "vkEnumerateInstanceExtensionProperties failed to find "
                  "the " VK_KHR_ANDROID_SURFACE_EXTENSION_NAME " extension.\n\n"
                  "Do you have a compatible Vulkan installable client driver (ICD) installed?\n"
                  "Please look at the Getting Started guide for additional information.\n",
                  "vkCreateInstance Failure" );
#elif defined( VK_USE_PLATFORM_XLIB_KHR )
    ...
    demo->gpu = physical_devices[demo->gpu_number];
    {
        ...
        fprintf( stderr, "Selected GPU %d: %s, type: %s\n", demo->gpu_number,
                 physicalDeviceProperties.deviceName, to_string( physicalDeviceProperties.deviceType ) );
    }
        ...
        for (uint32_t i = 0; i < device_extension_count; i++)
        {
            ...
            if (!strcmp( "VK_KHR_portability_subset", device_extensions[i].extensionName ))
            {
                demo->extension_names
                    [demo->enabled_extension_count++] = "VK_KHR_portability_subset";
            }
            assert( demo->enabled_extension_count < 64 );
        }
    ...
    if (demo->validate)
    {
        ...
        demo->CreateDebugUtilsMessengerEXT = (PFN_vkCreateDebugUtilsMessengerEXT)
            vkGetInstanceProcAddr( demo->inst, "vkCreateDebugUtilsMessengerEXT" );
        demo->DestroyDebugUtilsMessengerEXT = (PFN_vkDestroyDebugUtilsMessengerEXT)
            vkGetInstanceProcAddr( demo->inst, "vkDestroyDebugUtilsMessengerEXT" );
        demo->SubmitDebugUtilsMessageEXT = (PFN_vkSubmitDebugUtilsMessageEXT)
            vkGetInstanceProcAddr( demo->inst, "vkSubmitDebugUtilsMessageEXT" );
        demo->CmdBeginDebugUtilsLabelEXT = (PFN_vkCmdBeginDebugUtilsLabelEXT)
            vkGetInstanceProcAddr( demo->inst, "vkCmdBeginDebugUtilsLabelEXT" );
        demo->CmdEndDebugUtilsLabelEXT = (PFN_vkCmdEndDebugUtilsLabelEXT)vkGetInstanceProcAddr( demo->inst, "vkCmdEndDebugUtilsLabelEXT" );
        demo->CmdInsertDebugUtilsLabelEXT = (PFN_vkCmdInsertDebugUtilsLabelEXT)
            vkGetInstanceProcAddr( demo->inst, "vkCmdInsertDebugUtilsLabelEXT" );
        demo->SetDebugUtilsObjectNameEXT = (PFN_vkSetDebugUtilsObjectNameEXT)
            vkGetInstanceProcAddr( demo->inst, "vkSetDebugUtilsObjectNameEXT" );
        if (NULL == demo->CreateDebugUtilsMessengerEXT ||
            NULL == demo->DestroyDebugUtilsMessengerEXT || NULL == demo->SubmitDebugUtilsMessageEXT ||
            NULL == demo->CmdBeginDebugUtilsLabelEXT || NULL == demo->CmdEndDebugUtilsLabelEXT ||
            NULL == demo->CmdInsertDebugUtilsLabelEXT || NULL == demo->SetDebugUtilsObjectNameEXT)
        {
...
static VkSurfaceFormatKHR pick_surface_format( const VkSurfaceFormatKHR *surfaceFormats, uint32_t count )
{
    ...
    for (uint32_t i = 0; i < count; i++)
    {
        ...
        if (format == VK_FORMAT_R8G8B8A8_UNORM || format == VK_FORMAT_B8G8R8A8_UNORM ||
            ...
            format == VK_FORMAT_A2R10G10B10_UNORM_PACK32 || format == VK_FORMAT_R16G16B16A16_SFLOAT)
        {
            return surfaceFormats[i];
        }
    }

    printf( "Can't find our preferred formats... Falling back to first exposed format. Rendering "
            "may be incorrect.\n" );
...
static void registry_handle_global( void *data, struct wl_registry *registry, uint32_t id,
                                    const char *interface, uint32_t version UNUSED )
{
    struct demo *demo = data;
    ...
    if (strcmp( interface, wl_compositor_interface.name ) == 0)
    {
        ...
        {
            fprintf( stderr, "Wayland compositor doesn't support VK_KHR_incremental_present, "
                             "disabling.\n" );
            demo->VK_KHR_incremental_present_enabled = false;
        }
    }
...
static void demo_init( struct demo *demo, int argc, char **argv )
{
    ...
    for (int i = 1; i < argc; i++)
    {
        ...
        snprintf( usage, length + 1, message, APP_SHORT_NAME, VK_PRESENT_MODE_IMMEDIATE_KHR,
...
static WNDCLASSW wc =
{
    ...
    .lpfnWndProc = (WNDPROC)vulkan_window_proc,
    .lpszClassName = L"Vulkan",
};

HWND vulkan_create_window( const WCHAR *name, RECT *geometry, BOOL fullscreen )
{
    DWORD ex_style = WS_EX_APPWINDOW | WS_EX_WINDOWEDGE, style = WS_OVERLAPPEDWINDOW;
    char *argv[] = {(char *)"vulkan"};
    ...
    demo_init( data, 1, argv );

    data->window = CreateWindowExW( ex_style, wc.lpszClassName, name, WS_CLIPSIBLINGS | WS_CLIPCHILDREN | style,
                                    geometry->left, geometry->top, window_rect.right - window_rect.left,
                                    window_rect.bottom - window_rect.top, NULL, NULL, NULL, NULL );
    SetWindowLongPtrW( data->window, GWLP_USERDATA, (UINT_PTR)data );

    data->connection = GetModuleHandleW( NULL );
    data->minsize.x = GetSystemMetrics( SM_CXMINTRACK );
    data->minsize.y = GetSystemMetrics( SM_CYMINTRACK ) + 1;

    demo_init_vk_swapchain( data );
    demo_prepare( data );

    return data->window;
}

void vulkan_destroy_window( HWND hwnd )
{
    struct demo *data = (void *)GetWindowLongPtrW( hwnd, GWLP_USERDATA );

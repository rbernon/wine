...
static void x11_dwm_display_poll( struct display *iface, int req_fd )
{
    ...
    for (;;)
    {
        ...
        while (poll( fds, req_fd ? ARRAY_SIZE(fds) : 1, -1) <= 0) {}

        if (fds[0].revents & POLLOUT) xcb_flush( display->xcb );
        ...
        if (req_fd && (fds[1].revents & POLLIN)) break;
    }
}
...

{
    ...
    display->output = xcb_generate_id( display->xcb );

    cookie = xcb_create_window( display->xcb, XCB_COPY_FROM_PARENT, display->output, display->root, 0, 0,
                                (rect.right - rect.left) / 2, (rect.bottom - rect.top) / 2, 0,
                                XCB_WINDOW_CLASS_INPUT_OUTPUT, XCB_COPY_FROM_PARENT, 0, NULL );
    if (!ERR_ON(dwmsrv)) xcb_discard_reply( display->xcb, cookie.sequence );

    cookie = xcb_map_window( display->xcb, display->output );
    if (!ERR_ON(dwmsrv)) xcb_discard_reply( display->xcb, cookie.sequence );

    visual = xcb_aux_get_visualtype( display->xcb, display->screen_num, screen->root_visual );
    surface = cairo_xcb_surface_create( display->xcb, display->output, visual, (rect.right - rect.left) / 2, (rect.bottom - rect.top) / 2 );
    cairo_surface_set_device_scale( surface, 0.5, 0.5 );
    add_framebuffer_output( surface );

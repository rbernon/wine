    ...
    switch (type_get_type_detect_alias(t)) {
      case TYPE_ENUM:
        if (!define) fprintf(h, "enum %s", decl_name ? decl_name : "");
          ...
          t->written = TRUE;
          indentation++;
          write_enums( h, type_enum_get_values( t ), !t->namespace ? NULL : t->name );
          indent(h, -1);
          fprintf(h, "}");
        }
      ...
      case TYPE_ALIAS:
      {
        ...
        int in_namespace = ds && ds->type && ds->type->namespace;

...
void write_proxies( const statement_list_t *stmts )
{
    ...
    proc_format_string = format_proc_format_string(stmts, need_proxy, &proc_format_string_size);
  ...
  write_formatstringsdecl(proxy, indent, stmts, need_proxy, proc_format_string_size);
  write_stubdescproto();
  ...
  print_proxy( "    0,\n");
  print_proxy( "    {\n");
  print_proxy( "%s", proc_format_string );
  print_proxy( "        0x0\n");
  print_proxy( "    }\n");
  print_proxy( "};\n");
  print_proxy( "\n");

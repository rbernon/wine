/*
 * Copyright 2023 Rémi Bernon for CodWeavers
 *
...
static bool option_detect_all;
static char *program_name;

static XkbRF_VarDefsRec current_config;
static char *current_rules;
...
static void usage(void)
{
    ...
    fprintf( stderr, "  -a        Process all installed XKB layouts\n" );
}
...
static void option_callback( int optc, char *optarg )
{
    ...
    {
    case 'h': usage(); exit( 0 );
    case 'a': option_detect_all = true; break;
...
{
    return strmake( "{%#06x,%#06x,%#06x,%#06x,%#06x,%#06x,%#06x,%#06x}",
                    hashes->data[0], hashes->data[1], hashes->data[2], hashes->data[3],
                    hashes->data[4], hashes->data[5], hashes->data[6], hashes->data[7] );
}

static const char *dump_keymap_langid( const struct keymap *keymap )
{
    LANGID langid;
    if (!strcmp( keymap->layout, "af" )) langid = MAKELANGID(LANG_DARI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "al" )) langid = MAKELANGID(LANG_ALBANIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "am" )) langid = MAKELANGID(LANG_ARMENIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ara" )) langid = MAKELANGID(LANG_ARABIC, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "at" )) langid = MAKELANGID(LANG_GERMAN, SUBLANG_GERMAN_AUSTRIAN);
    else if (!strcmp( keymap->layout, "az" )) langid = MAKELANGID(LANG_AZERBAIJANI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ba" )) langid = MAKELANGID(LANG_BOSNIAN, SUBLANG_BOSNIAN_BOSNIA_HERZEGOVINA_CYRILLIC);
    else if (!strcmp( keymap->layout, "bd" )) langid = MAKELANGID(LANG_BANGLA, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "be" )) langid = MAKELANGID(LANG_FRENCH, SUBLANG_FRENCH_BELGIAN);
    else if (!strcmp( keymap->layout, "bg" )) langid = MAKELANGID(LANG_BULGARIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "br" )) langid = MAKELANGID(LANG_PORTUGUESE, 2);
    else if (!strcmp( keymap->layout, "brai" )) langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_DEFAULT);
    else if (!strcmp( keymap->layout, "bt" )) langid = MAKELANGID(LANG_TIBETAN, 3);
    else if (!strcmp( keymap->layout, "bw" )) langid = MAKELANGID(LANG_TSWANA, SUBLANG_TSWANA_BOTSWANA);
    else if (!strcmp( keymap->layout, "by" )) langid = MAKELANGID(LANG_BELARUSIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ca" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_CAN);
    else if (!strcmp( keymap->layout, "cd" )) langid = MAKELANGID(LANG_FRENCH, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "ch" )) langid = MAKELANGID(LANG_GERMAN, SUBLANG_GERMAN_SWISS);
    else if (!strcmp( keymap->layout, "cm" )) langid = MAKELANGID(LANG_FRENCH, 11);
    else if (!strcmp( keymap->layout, "cn" )) langid = MAKELANGID(LANG_CHINESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "cz" )) langid = MAKELANGID(LANG_CZECH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "de" )) langid = MAKELANGID(LANG_GERMAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "dk" )) langid = MAKELANGID(LANG_DANISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "dz" )) langid = MAKELANGID(LANG_TAMAZIGHT, SUBLANG_TAMAZIGHT_ALGERIA_LATIN);
    else if (!strcmp( keymap->layout, "ee" )) langid = MAKELANGID(LANG_ESTONIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "epo" )) langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_DEFAULT);
    else if (!strcmp( keymap->layout, "es" )) langid = MAKELANGID(LANG_SPANISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "et" )) langid = MAKELANGID(LANG_AMHARIC, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "fi" )) langid = MAKELANGID(LANG_FINNISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "fo" )) langid = MAKELANGID(LANG_FAEROESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "fr" )) langid = MAKELANGID(LANG_FRENCH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "gb" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_UK);
    else if (!strcmp( keymap->layout, "ge" )) langid = MAKELANGID(LANG_GEORGIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "gh" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "gn" )) langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_DEFAULT);
    else if (!strcmp( keymap->layout, "gr" )) langid = MAKELANGID(LANG_GREEK, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "hr" )) langid = MAKELANGID(LANG_CROATIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "hu" )) langid = MAKELANGID(LANG_HUNGARIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "id" )) langid = MAKELANGID(LANG_INDONESIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ie" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_EIRE);
    else if (!strcmp( keymap->layout, "il" )) langid = MAKELANGID(LANG_HEBREW, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "in" )) langid = MAKELANGID(LANG_HINDI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ir" )) langid = MAKELANGID(LANG_PERSIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "is" )) langid = MAKELANGID(LANG_ICELANDIC, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "it" )) langid = MAKELANGID(LANG_ITALIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "jp" )) langid = MAKELANGID(LANG_JAPANESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "jv" )) langid = MAKELANGID(LANG_INDONESIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ke" )) langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_DEFAULT);
    else if (!strcmp( keymap->layout, "kg" )) langid = MAKELANGID(LANG_KYRGYZ, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "kh" )) langid = MAKELANGID(LANG_KHMER, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "kz" )) langid = MAKELANGID(LANG_KAZAK, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "la" )) langid = MAKELANGID(LANG_LAO, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "latam" )) langid = MAKELANGID(LANG_SPANISH, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "lk" )) langid = MAKELANGID(LANG_SINHALESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "lt" )) langid = MAKELANGID(LANG_LITHUANIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "lv" )) langid = MAKELANGID(LANG_LATVIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ma" )) langid = MAKELANGID(LANG_ARABIC, SUBLANG_ARABIC_MOROCCO);
    else if (!strcmp( keymap->layout, "mao" )) langid = MAKELANGID(LANG_MAORI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "md" )) langid = MAKELANGID(LANG_ROMANIAN, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "me" )) langid = MAKELANGID(LANG_SERBIAN, SUBLANG_SERBIAN_MONTENEGRO_LATIN);
    else if (!strcmp( keymap->layout, "mk" )) langid = MAKELANGID(LANG_MACEDONIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ml" )) langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_DEFAULT);
    else if (!strcmp( keymap->layout, "mm" )) langid = MAKELANGID(0x55/*LANG_BURMESE*/, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "mn" )) langid = MAKELANGID(LANG_MONGOLIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "mt" )) langid = MAKELANGID(LANG_MALTESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "mv" )) langid = MAKELANGID(LANG_DIVEHI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "my" )) langid = MAKELANGID(LANG_MALAY, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ng" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "nl" )) langid = MAKELANGID(LANG_DUTCH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "np" )) langid = MAKELANGID(LANG_NEPALI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ph" )) langid = MAKELANGID(LANG_FILIPINO, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "pk" )) langid = MAKELANGID(LANG_URDU, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "pl" )) langid = MAKELANGID(LANG_POLISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "pt" )) langid = MAKELANGID(LANG_PORTUGUESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "ro" )) langid = MAKELANGID(LANG_ROMANIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "rs" )) langid = MAKELANGID(LANG_SERBIAN, SUBLANG_SERBIAN_LATIN);
    else if (!strcmp( keymap->layout, "ru" )) langid = MAKELANGID(LANG_RUSSIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "se" )) langid = MAKELANGID(LANG_SWEDISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "si" )) langid = MAKELANGID(LANG_SLOVENIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "sk" )) langid = MAKELANGID(LANG_SLOVAK, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "sn" )) langid = MAKELANGID(LANG_WOLOF, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "sy" )) langid = MAKELANGID(LANG_SYRIAC, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "tg" )) langid = MAKELANGID(LANG_FRENCH, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "th" )) langid = MAKELANGID(LANG_THAI, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "tj" )) langid = MAKELANGID(LANG_TAJIK, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "tm" )) langid = MAKELANGID(LANG_TURKMEN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "tr" )) langid = MAKELANGID(LANG_TURKISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "tw" )) langid = MAKELANGID(LANG_CHINESE, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "tz" )) langid = MAKELANGID(LANG_SWAHILI, SUBLANG_CUSTOM_UNSPECIFIED);
    else if (!strcmp( keymap->layout, "ua" )) langid = MAKELANGID(LANG_UKRAINIAN, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "us" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "uz" )) langid = MAKELANGID(LANG_UZBEK, 2);
    else if (!strcmp( keymap->layout, "vn" )) langid = MAKELANGID(LANG_VIETNAMESE, SUBLANG_DEFAULT);
    else if (!strcmp( keymap->layout, "za" )) langid = MAKELANGID(LANG_ENGLISH, SUBLANG_ENGLISH_SOUTH_AFRICA);
    else langid = MAKELANGID(LANG_NEUTRAL, SUBLANG_CUSTOM_UNSPECIFIED);
    return strmake( "%#06x", langid );
};
...
static void compute_keymap_hashes( Display *display, int group, struct keymap_hashes *hashes )
{
    ...
    printf( "[" );
    for (keyc = keyc_min; keyc <= keyc_min + 0x7f; keyc++)
    {
        if (!XkbLookupKeySym( display, keyc, group | ShiftMask, &dummy, &keysym )) printf( " 0x000000ff," );
        else printf( " 0x%08x,", (unsigned int)keysym );
    }
    printf( "],\n" );
...
static int keymap_compare( const void *a, const void *b )
{
    const struct keymap *a_km = a, *b_km = b;
    int r;

    if ((r = !!a_km->variant - !!b_km->variant)) return r;
    if ((r = !!strcmp( a_km->layout, "us" ) - !!strcmp( b_km->layout, "us" ))) return r;
    if ((r = !!strcmp( a_km->layout, "de" ) - !!strcmp( b_km->layout, "de" ))) return r;
    if ((r = !!strcmp( a_km->layout, "fr" ) - !!strcmp( b_km->layout, "fr" ))) return r;

    if ((r = strcmp( a_km->layout, b_km->layout ))) return r;
    if (!a_km->variant) return 0;

    if ((r = strlen( a_km->variant ) - strlen( b_km->variant ))) return r;
    return strcmp( a_km->variant, b_km->variant );
};

static void get_current_config( char **rules, XkbRF_VarDefsRec *config )
{
    if (!XkbRF_GetNamesProp( display, rules, config ) || !rules)
        fatal_error( "Failed to read keyboard layout!\n" );
}

static void set_current_config( char *rules, XkbRF_VarDefsRec *config )
{
    if (!XkbRF_SetNamesProp( display, rules, config ))
        fatal_error( "Failed to change keyboard layout!\n" );
}

static const struct keymap *find_known_keymap( const struct keymap *keymaps, unsigned int keymap_count,
                                               struct keymap_hashes *hashes, unsigned int *score )
{
    ...
    *score = 9;
    ...
    {
        if (!memcmp( hashes, &keymaps[i].hashes, sizeof(keymaps[i].hashes) )) break;
        for (count = 0, j = 0; j < ARRAY_SIZE(keymaps[i].hashes.data); ++j) count += keymaps[i].hashes.data[j] == hashes->data[j];
        if (best_count >= count) continue;
        best_keymap = keymaps + i;
        best_count = count;
    }

    if (i < keymap_count) return keymaps + i;
    *score = best_count;
    return best_keymap;
}

static bool detect_layout_variant( XkbRF_RulesPtr xkb_rules, const char *rules, char *layout,
                                   char *variant, struct keymap_hashes *hashes )
{
    XkbRF_VarDefsRec config = {.layout = layout, .variant = variant, .model = (char *)"pc105"};
    XkbComponentNamesRec components = {0};
    bool ret;

    XkbRF_GetComponents( xkb_rules, &config, &components );

    if ((ret = !!XkbGetKeyboardByName( display, XkbUseCoreKbd, &components, XkbGBN_AllComponentsMask,
                                       XkbGBN_AllComponentsMask & (~XkbGBN_GeometryMask), True )))
        ret = !!XkbRF_SetNamesProp( display, (char *)rules, &config );

    printf("%s %s\n", layout, variant);
    if (ret) compute_keymap_hashes( display, 0, hashes );
    return ret;
}

static void restore_current_config(void)
{
    set_current_config( current_rules, &current_config );
}

int main( int argc, char **argv )
{
    XkbRF_VarDescPtr layout, layout_end, variant, variant_end;
    const char *rules_list[] = {"base"};
    struct keymap *keymaps = NULL;
    unsigned int i, j, count = 0;
    XkbRF_RulesPtr xkb_rules;
    char rules_path[1024];

    program_name = argv[0];
    parse_options( argc, argv, "hao:", NULL, 0, option_callback );

    if (!(display = XkbOpenDisplay( NULL, NULL, NULL, NULL, NULL, NULL )))
        fatal_error( "Cannot open display!\n" );

    if (!option_detect_all)
    {
        struct keymap_hashes hashes = {0};
        const struct keymap *keymap;
        unsigned int score;

        printf( "Current Xkb group hashes:\n");

        compute_keymap_hashes( display, 0, &hashes );
        if ((keymap = find_known_keymap( all_keymaps, ARRAY_SIZE(all_keymaps), &hashes, &score )))
            printf("Found %s \"%s\" with score %u\n", keymap->layout, keymap->description, score);
        printf( "0: {%s}\n", dump_keymap_hashes( &hashes ) );
        compute_keymap_hashes( display, 0x2000, &hashes );
        if ((keymap = find_known_keymap( all_keymaps, ARRAY_SIZE(all_keymaps), &hashes, &score )))
            printf("Found %s \"%s\" with score %u\n", keymap->layout, keymap->description, score);
        printf( "1: {%s}\n", dump_keymap_hashes( &hashes ) );
        compute_keymap_hashes( display, 0x4000, &hashes );
        if ((keymap = find_known_keymap( all_keymaps, ARRAY_SIZE(all_keymaps), &hashes, &score )))
            printf("Found %s \"%s\" with score %u\n", keymap->layout, keymap->description, score);
        printf( "2: {%s}\n", dump_keymap_hashes( &hashes ) );
        compute_keymap_hashes( display, 0x6000, &hashes );
        if ((keymap = find_known_keymap( all_keymaps, ARRAY_SIZE(all_keymaps), &hashes, &score )))
            printf("Found %s \"%s\" with score %u\n", keymap->layout, keymap->description, score);
        printf( "3: {%s}\n", dump_keymap_hashes( &hashes ) );
    ...
    get_current_config( &current_rules, &current_config );
    atexit( restore_current_config );
    init_signals( exit_on_signal );

    for (i = 0; i < ARRAY_SIZE(rules_list); ++i)
    {
        sprintf( rules_path, "/usr/share/X11/xkb/rules/%s", rules_list[i] );
        if (!(xkb_rules = XkbRF_Load( rules_path, NULL, true, true )))
        {
            fprintf( stderr, "Failed to open rules %s\n", rules_path );
            continue;
        }

        layout_end = xkb_rules->layouts.desc + xkb_rules->layouts.num_desc;
        variant_end = xkb_rules->variants.desc + xkb_rules->variants.num_desc;

        for (layout = xkb_rules->layouts.desc; layout != layout_end; layout++)
        {
            struct keymap_hashes hashes;
            if (!detect_layout_variant( xkb_rules, rules_list[i], layout->name, (char *)"", &hashes )) continue;

            keymaps = xrealloc( keymaps, ++count * sizeof(*keymaps) );
            memset( &keymaps[count - 1], 0, sizeof(*keymaps) );
            keymaps[count - 1].layout = xstrdup( layout->name );
            keymaps[count - 1].description = xstrdup( layout->desc );
            memcpy( &keymaps[count - 1].hashes, &hashes, sizeof(hashes) );
        }

        variant = xkb_rules->variants.desc;
        for (layout = xkb_rules->layouts.desc; layout != layout_end; layout++)
        {
            unsigned int len = strlen( layout->name );
            struct keymap_hashes hashes;

            for (; variant != variant_end; variant++)
            {
                const struct keymap *keymap;
                unsigned int score;

                if (strncmp( variant->desc, layout->name, len ) || variant->desc[len] != ':') break;
                if (!detect_layout_variant( xkb_rules, rules_list[i], layout->name, variant->name, &hashes )) continue;
                if ((keymap = find_known_keymap( keymaps, count, &hashes, &score )) && score > 4)
                {
                    if (score < 9)
                        fprintf( stderr, "Ignoring layout %s:%s, matching %s:%s \"%s\" with score %u\n",
                                 layout->name, variant->name, keymap->layout, keymap->variant, keymap->description, score );
                    continue;
                }
                ...
                memset( &keymaps[count - 1], 0, sizeof(*keymaps) );
                keymaps[count - 1].layout = xstrdup( layout->name );
                keymaps[count - 1].variant = xstrdup( variant->name );
                keymaps[count - 1].description = xstrdup( variant->desc + strlen( layout->name ) + 2 /*": "*/ );
                memcpy( &keymaps[count - 1].hashes, &hashes, sizeof(hashes) );
            }
        }
    }

    qsort( keymaps, count, sizeof(*keymaps), keymap_compare );

    for (i = 0; i < count; ++i)
    {
        if (!keymaps[i].hashes.data[0]) continue;

        printf( "{.langid = %s, .hashes = {%s}, ", dump_keymap_langid( &keymaps[i] ), dump_keymap_hashes( &keymaps[i].hashes ) );

        if (!keymaps[i].variant) printf( ".layout = \"%s\", ", keymaps[i].layout );
        else printf( ".layout = \"%s:%s\", ", keymaps[i].layout, keymaps[i].variant );
        printf( ".description = \"%s\"},\n", keymaps[i].description );

        for (j = i + 1; j < count; ++j)
        {
            if (memcmp( &keymaps[i].hashes, &keymaps[j].hashes, sizeof(keymaps[i].hashes) )) continue;

            printf( "/* same as: " );
            if (!keymaps[j].variant) printf( "%s", keymaps[j].layout );
            else printf( "%s:%s", keymaps[j].layout, keymaps[j].variant );
            printf( " \"%s\" */\n", keymaps[j].description );

            keymaps[j].hashes.data[0] = 0;
        }
    }
